...
BOOL CExample1Dlg::OnInitDialog()
{
	...
	int ret = WSAStartup(version, &wsaData);//加载Winsock DLL的相应版本
	if(ret != 0)
	{
...
LRESULT CExample1Dlg::OnServerMessage(WPARAM wParam, LPARAM lParam)
{
	...
	{
		case FD_ACCEPT:
			//接受连接创建连接sock，存储client与SVR链接进入链表
			socket= accept(m_server.m_hSocket,NULL,NULL);
			length=m_connectionList.GetCount();
			//"一个游客进入了聊天室" 发到已经存储的所有client，不包括触发当前ACCEPT的client
			for(i=0;i<length;i++)
			{
				ts=m_connectionList.GetAt(m_connectionList.FindIndex(i));//返回是链表存储的对象，sock
				s[0]=NULL;
				strcat(s,"一个游客进入了聊天室");
			...
			m_connectionList.AddHead(socket);//当前accept的client的连接sock存储到链表
			return 0;
		...
		case FD_READ:
			...
			for(i=0;i<length;i++)
			{
				...
				if(socket==wParam)//定位到发送client，recv
				{
